...
ALTER TABLE `maalquran_data`.`sura` CHANGE `juz` `startjuz` smallint(3) DEFAULT NULL;
ALTER TABLE `maalquran_data`.`sura` CHANGE `alljuz` `endjuz` smallint(3) DEFAULT NULL;

UPDATE `maalquran_data`.`sura` SET `maalquran_data`.`sura`.`startjuz` = ( SELECT `maalquran_data`.`quran_ayat`.`juz` FROM `maalquran_data`.`quran_ayat` WHERE `maalquran_data`.`quran_ayat`.`sura` = `maalquran_data`.`sura`.`index` ORDER BY `maalquran_data`.`quran_ayat`.`index` ASC LIMIT 1);
UPDATE `maalquran_data`.`sura` SET `maalquran_data`.`sura`.`endjuz` = ( SELECT `maalquran_data`.`quran_ayat`.`juz` FROM `maalquran_data`.`quran_ayat` WHERE `maalquran_data`.`quran_ayat`.`sura` = `maalquran_data`.`sura`.`index` ORDER BY `maalquran_data`.`quran_ayat`.`index` DESC LIMIT 1);
