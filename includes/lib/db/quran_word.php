...
class quran_word
{
	public static function get($_where, $_option = [])
	{
		...
		$_option['order'] = ' ORDER BY `quran_word`.`id` ASC ';
		return \dash\db\config::public_get('quran_word', $_where, $_option);
	}
	...
	public static function get_first_word($_where, $_order = 'ASC')
	{
		...
		$query  = "SELECT * FROM quran_word WHERE $_where ORDER BY quran_word.id $_order LIMIT 1";
	...
	{
		$query  = "SELECT * FROM quran_word WHERE quran_word.id = '$_id' LIMIT 1";
		$result = \dash\db::get($query, null, true, \lib\db\db_data_name::get());
		return $result;
	}
	...
	{
		$query  = "SELECT * FROM quran_word WHERE quran_word.id >= '$_from' AND quran_word.id <= $_to";
